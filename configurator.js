(function () {
  ...
  function updateSummary() {
    ...
    if (priceEl) {
      if (cfg.priceKnown) {
        const price = (state.antrieb && cfg.bundlePrice) ? cfg.bundlePrice : cfg.basePrice;
        const note = state.antrieb ? 'inkl. Antriebsset' : 'ohne Antrieb — Set optional dazu wählbar';
        priceEl.innerHTML = fmtPrice(price) + `<small>${note}</small>`;
      } else {
        const note = state.antrieb ? 'inkl. gewünschtem Antriebsset' : 'Antriebsset optional dazu wählbar';
        priceEl.innerHTML = 'Auf Anfrage' + `<small>${note}</small>`;
      }
    }
